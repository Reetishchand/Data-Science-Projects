...
def draw_preds(img_path, preds):
    ...
    for pred in preds:
        ...
        draw.text((loc[3], loc[0] - 30), name,font=ImageFont.truetype('/Library/Fonts/Arial.ttf', 30))
    source_img.show()

if __name__ == "__main__":
    classifier = train("images/train", "", 3)
    exit = False
    while not exit :
        image = input("enter the path : \n")
        try:
            path = "images/test/" + image
            draw_preds(path, predict(path, classifier, ""))
